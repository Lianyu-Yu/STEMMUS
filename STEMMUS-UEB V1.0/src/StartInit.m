...
global COR CORh Theta_V Theta_g Se KL_h DTheta_LLh KfL_h DTheta_UUh hThmrl Tr Hystrs KIT RHOI RHOL
hd=-1e7;hm=-9899;
...
Elmn_Lnth=0;
Dmark=0;
FOS=[0.388 0.441 0.443 0.447 0.656 0.656];  %[0.273 0.253 0.355 0.382 0.398] fraction of sand
FOC=[0.094 0.09 0.10 0.106 0.03 0.03];  %[0.012 0.021 0.006 0.005 0] fraction of clay
FOSL=1-FOC-FOS;
Vol_qtz=FOS;
MSOC=[0.138 0.084 0.054 0.018 0.01 0.01]; %[0.043 0.014 0.003 0.002 0.0003 0.0003];%[0.138 0.084 0.054 0.018 0.01 0.01];   %;[0.138 0.084 0.054 0.018 0.01] mass fraction of soil organic matter
...
Coef_Lamda=1./[3.98 4.178 4.3 4.4 3.4 3.4]; %1./[3.98 4.078 4.2 4.4 3.4 3.4];
...
for J=1:6
    ...
    m(J)=1-1/n(J);
    XK(J)=0.09; %0.11 This is for silt loam; For sand XK=0.025
    XWILT(J)=Theta_r(J)+(Theta_s(J)-Theta_r(J))/(1+abs(Alpha(J)*(-1.5e4))^n(J))^m(J);
    XCAP(J)=Theta_r(J)+(Theta_s(J)-Theta_r(J))/(1+abs(Alpha(J)*(-336))^n(J))^m(J);
    VPERS(J)=FOS(J)*(1-POR(J));
    VPERSL(J)=FOSL(J)*(1-POR(J));
    VPERC(J)=FOC(J)*(1-POR(J));
    if SWCC==0
        if CHST==0
            Phi_s(J)=Phi_S(J);
            Lamda(J)=Coef_Lamda(J);
        end
        Theta_s(J)=Theta_s_ch(J);
    end
end
Ksh(1:6)=[2.21*1e-4  2.21*1e-4 4.37*1e-4 0.36*1e-4 4.15*1e-4 4.15*1e-4];
BtmKsh=Ksh(6);
Ksh0=Ksh(1);
if ~Eqlspace
...
BtmPg=67197.850*10;     % Atmospheric pressure at the bottom (Pa), set fixed
