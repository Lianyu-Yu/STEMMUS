...
Nmsrmn=5068*10;       %Nmsrmn=2568*100;  Here, it is made as big as possible, in case a long simulation period containing many time step is defined.

DURTN=60*30*5068;     % Duration of simulation period;
...
Tot_Depth=160;           % Unit is cm. it should be usually bigger than 0.5m. Otherwise,
...
%>
%>
global alpha_h bx Srt rwuef Ts_msr Tb_msr Theta_U Theta_UU Theta_I Theta_II KfL_h RHOI KfL_T Tbtm r_a_SOIL Rn_SOIL EVAP XCAP
global Ta_msr RH_msr Rn_msr WS_msr G_msr Pg_msr HourInput Rns_msr SH_msr LE_msr DRHOVZ DTDZ DhDZ
global DLemdDZ DqLDZ DqVDZ DQaDZ SqVDZ LemdaBAR csBAR HC DTheta_LLhBAR LHS csBAR1 HC1 DTheta_LLT csBAR2 HC2
alpha_h=zeros(mL,nD);      %root water uptake
bx=zeros(mL,nD);
Srt=zeros(mL,nD);DhDZ=zeros(mL,1);DTDZ=zeros(mL,1);DRHOVZ=zeros(mL,1);
Ratio_ice=zeros(mL,nD);DTheta_LLT=zeros(mL,nD);
...
QL_a=zeros(mL,1);
DLemdDZ=zeros(mL,1); DqLDZ=zeros(mL,1); DqVDZ=zeros(mL,1); DQaDZ=zeros(mL,1); SqVDZ=zeros(mL,1);
LemdaBAR=zeros(mL,1);csBAR=zeros(mL,1); HC=zeros(mL,1);DTheta_LLhBAR=zeros(mL,1); LHS=zeros(mL,1);csBAR1=zeros(mL,1);HC1=zeros(mL,1);
csBAR2=zeros(mL,1);HC2=zeros(mL,1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%% 172   27%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SaturatedK=[1.45*1e-4  1.45*1e-4 0.94*1e-4 0.94*1e-4 0.68*1e-4 0.68*1e-4];%[2.67*1e-3  1.79*1e-3 1.14*1e-3 4.57*1e-4 2.72*1e-4];      %[2.3*1e-4  2.3*1e-4 0.94*1e-4 0.94*1e-4 0.68*1e-4] 0.18*1e-4Saturation hydraulic conductivity (cm.s^-1);
SaturatedMC=[0.5 0.441 0.48 0.48 0.42 0.42];                              % 0.42 0.55 Saturated water content;
ResidualMC=[0.041 0.035 0.039 0.036 0.045 0.045];                               %0.037 0.017 0.078 The residual water content of soil;
Coefficient_n=[1.365 1.332 1.3618 1.3329 1.59 1.59];                            %1.2839 1.3519 1.2139 Coefficient in VG model;
Coefficient_Alpha=[0.04139 0.04139 0.04139 0.04139 0.075 0.075];                   % 0.02958 0.007383 Coefficient in VG model;
porosity=[0.616 0.616 0.572 0.572 0.495 0.495];                                      % Soil porosity;
...
if SWCC==0
    ...
    InitT0=	-1.75;  %-1.75estimated soil surface temperature-1.762
    InitT1=	-0.68;
    InitT2=	0.264;
    InitT3=	0.905;
    InitT4=	4.29;
    ...
    InitT6=	6.033;
    BtmT=6.033;  %9 8.1
    InitX0=	0.0793;  %0.0793
    InitX1=	0.0925; % Measured soil liquid moisture content
    InitX2=	0.182; %0.182
    InitX3=	0.2058;
    InitX4=	0.185; %0.14335
    InitX5=	0.0845;
    InitX6=	0.078;
    BtmX=0.078;%0.05;    % The initial moisture content at the bottom of the column.
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
